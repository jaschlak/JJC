"""Create the JJC V1 lower body shell."""
...
def run(_context):
    ...
    try:
        ...
        # Parameters from Resources/6_lower_body_shell_spec.md.
        ...
        collar_side_inner_diameter_mm = collar_side_outer_diameter_mm - (wall_thickness_mm * 2.0)
        ...
        bottom_sketch.name = 'Wide body-side shell profile on XY plane'
        ...
        top_plane.name = 'Collar-side shell plane'

        top_sketch = component.sketches.add(top_plane)
        top_sketch.name = 'Collar-side shell profile'
        top_sketch.sketchCurves.sketchCircles.addByCenterRadius(
            center,
            mm_to_cm(collar_side_outer_diameter_mm / 2.0)
        )
        top_sketch.sketchCurves.sketchCircles.addByCenterRadius(
            center,
            mm_to_cm(collar_side_inner_diameter_mm / 2.0)
        )
        ...
        loft.name = 'Lower body shell loft'

        body = loft.bodies.item(0)
        body.name = 'Lower body shell'
